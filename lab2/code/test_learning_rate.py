...
def run(neural_network, init_weights):
    ...
    print(f'{learning_rate:.{learning_rate_decimals}f},{train_accuracy:.2f},{test_accuracy:.2f}')
...
if __name__ == '__main__':
    ...
    learning_rate_decimals = 5
    run(nn, init_w)

    learning_rate = 0.0001
    learning_rate_decimals = 4
    run(nn, init_w)

    learning_rate = 0.001
    learning_rate_decimals = 3
    run(nn, init_w)

    learning_rate = 0.01
    learning_rate_decimals = 2
    run(nn, init_w)

    learning_rate = 0.1
    for i in range(20):
        delta_learning_rate = 0.1
        learning_rate_decimals = 1
        run(nn, init_w)
        learning_rate += delta_learning_rate

    learning_rate = 10
    learning_rate_decimals = 0
    run(nn, init_w)
